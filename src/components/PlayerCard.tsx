...
export function PositionIcon({ position, className = "w-5 h-5 text-[8px]" }: { position: string, className?: string }) {
  const p = position.replace('-', '/');
  
  if (p === 'ALL' || p === 'STAR') {
    return (
      <div className={`relative rounded-full overflow-hidden shadow-sm border border-white/40 flex items-center justify-center shrink-0 ${className}`}>
