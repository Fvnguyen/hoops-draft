      ...
      <div className="max-w-4xl mx-auto">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div>
            ...
            {/* Active Bonuses */}
            {season.currentGame > 0 && (
              <div className="mt-4">
                <h2 className="text-xs font-bold uppercase tracking-widest text-stone-400 mb-2">Your Bonuses</h2>
                <div className="bg-white rounded-xl border border-stone-200 shadow-sm p-3 text-xs text-stone-600">
                  {/* We'd need to compute bonuses here — show them after first game */}
                  {season.schedule[0]?.result && (
                    <div className="space-y-1">
                      {(season.schedule[0].result.homeBonuses.activeSynergies.length > 0 || season.schedule[0].result.homeBonuses.activePlays.length > 0) ? (
                        <>
                          {season.schedule[0].result.homeBonuses.activeSynergies.map(s => (
                            <div key={s.name} className="flex items-center gap-1">
                              <span className="text-emerald-600">✦</span> {s.name}
                            </div>
                          ))}
                          {season.schedule[0].result.homeBonuses.activePlays.filter(p => p.activated !== 'none').map(p => (
                            <div key={p.name} className="flex items-center gap-1">
                              <span className={p.activated === 'full' ? 'text-amber-500' : 'text-stone-400'}>▶</span> {p.name}
                              <span className="text-[10px] text-stone-400">({p.activated})</span>
                            </div>
                          ))}
                          {season.schedule[0].result.homeBonuses.playstyle.map(p => (
                            <div key={p} className="text-stone-400 italic">🏀 {p}</div>
                          ))}
                        </>
                      ) : (
                        <div className="text-stone-400 italic">No active synergies</div>
                      )}
                    </div>
                  )}
                </div>
