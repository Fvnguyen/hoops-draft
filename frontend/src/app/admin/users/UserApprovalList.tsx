...
type User = { id: string; email: string; display_name: string; status: string; role: string; created_at: string };
...
export function UserApprovalList({ initialUsers }: { initialUsers: User[] }) {
  ...
  return <div className="space-y-3">
    ...
    {users.map((user) => <div key={user.id} className="flex flex-wrap items-center justify-between gap-3 border border-stone-700 bg-stone-900/70 p-4">
      <div><p className="font-bold text-white">{user.display_name}</p><p className="text-sm text-stone-400">{user.email}</p></div>
