...
File: `docs/plans/plan_render_and_engine_perf_2026-09-21.md`. Status: planned. Sequence: 13 in `docs/ROADMAP.md`.
Depends on: nothing.
Files owned (all under `frontend/src/` unless noted): `engine/{game,lineup,challenge,season,cards,rosterStats,
synergies,deckbuilder,gameTypes,rotation,shot,possession,boxscore,teamInfo}.ts` (last 6 new),
...
`frontend/scripts/bench-engine.ts` (new), `docs/ARCHITECTURE.md`, `tests/unit/*`.

## Goal

The deck builder, game view and 82:0 sim do far more work per render or possession than needed, and the two
biggest files (`game.ts` 1,552 lines, `DeckBuilder.tsx` 1,534 lines) are hard to change safely. Cut wasted engine
...
identical for a fixed seed throughout: `npm run balance -- 500 --seed 42` unchanged start to finish.
...
- D5 (E6, top). `playNextGame` (`season.ts:193`) derives each matchup's seed mixSeed-style from `Season.seed`
  ...
  `new Date()`(183) become caller args. Conflict: sibling `sync_outbox` changes the season id FORMAT, not the
  source this task touches; coordinate if it lands first.
- D6 (E7, mid, before D1/D2). Split `game.ts` into `gameTypes.ts` (interfaces, ~44-199), `rotation.ts`
  ...
  `boxscore.ts` (emptyBoxScore/boxScoreThrough:1416-1524, new shared `accumulateBoxRow(totals,row)` replacing
  the duplicate at `season.ts:541-576`/`challenge.ts:392-418`), `teamInfo.ts` (buildTeamInfo:1051-1133). `game.ts`
...
- D8 (U1, mid). `hooks/useRosterBuilder.ts` (new): reducer owning `depthChart`/`rosterPlayers`/`activePlays`/
  ...
  `assignRole`/`clearRole`/`clear`/`undo`, pure body in `engine/deckbuilder.ts` (initial state = its existing `initBuilderState`); replaces the setter-inside-setter
  at `DeckBuilder.tsx:520-527` (repeats at `:632-640`). `hooks/useDockLayout.ts` (new) owns `tier` (from
  `containerWidth`, set by the `ResizeObserver` at `284-293`) plus docked/drawer flags. `SaveRosterModal.tsx`
  ...
  re-run is replaced by an `archetypeStatuses` prop fed from `DeckBuilder.tsx`'s existing `useMemo` (line 796);
  delete the dead `bonuses` prop (`TopKPIBand.tsx:163`) and its `calcTeamBonuses` call (`DeckBuilder.tsx:787`,
  grep other reads first), trim its and `calcRosterShotDiet`'s unused params (D7). Conflict: sibling
  `mobile_load` edits `DeckBuilder.tsx:525` (avatar URL) inside this plan's `520-527` fix; start after it lands,
  or take the disjoint hunk.
...
- D10 (U4+U6, mid). `PlayerCard.tsx`: delete `PlayCard`'s dead `status`/`players`/`selectedRoleId`/`onRole*`/
  `evaluation` props and the branches they gate (`1162-1264` role-list view, `1349-1357` requirement list; zero
  callers, confirmed at `DraftRoom.tsx:562`, `rosters/page.tsx:330`, `PackRevealCard.tsx:52`). Move
  `PlayBoardGraphicGeneric`/`PlayMotifSvg`/`PlayBoardGraphic` (`806-1013`) to new `PlayArt.tsx`; move
  `useLongPressPreview` (inline, `19-43`) to new `hooks/useLongPressPreview.ts` and attach it in the compact
  branch (`726-770`, unattached today), in `RosterPlayerRow` and `StarterFront`; move the Mythic gem `<style>`
  (`336-343`) into `globals.css`; `useHoverPreview.ts:60-66`'s `onMouseEnter` ignores coarse pointers. Conflict:
  sibling `mobile_load` owns this file's blur/back-face work, start after it lands. The touch-attach half needs
...
- D11 (U5, mid). `rosters/page.tsx`: the 5-card roster summary renders `PlayerCardFront` not the full flip
  `PlayerCard`; the per-roster stagger (`delay: i*0.1`, line 245, compounding across the whole list) caps at
  `Math.min(i*0.1, 1.0)`; roster wrapper gets `content-visibility:auto`. `TopNav.tsx`: one `ProfileSummary`
  replaces the duplicated block at `116-141`/`253-273`; `useUserSeasonStats` (`107` and `207`) called once.
...
T1 (mid, wave 0, driver). New `bench-engine.ts`, times `simulateHalf` half 1/2 (post-trade roster); locks the
`events` option, `accumulateBoxRow`, `useRosterBuilder` action-type contracts. Done: script runs, baseline ms
(~180/~320) printed.
T2 (mid). D6 split. Done: `game.ts` < 500 lines, `tsc`/`engine-purity.test.ts`/`npm test` green, `balance` == baseline.
T3 (top). D1. Done: `npm test` green, `balance` byte-identical, half 2 <= 140 ms.
...
T10 (mid, after T9 and `mobile_load`). D8 layout/sidebars/wiring. Done: `DeckBuilder.tsx` < 700 lines, spec zero
...
T12 (mid, after `mobile_load`). D10 trim/extractions/gate. Done: `visual.spec.ts` snapshots unchanged, `tsc`/
`lint` clean.
...
sequence, alongside T11. Wave 5 (after `mobile_load` lands): T12, then T13 and T14. Agents never run git; the
...
`bench-engine.ts` committed, half 2 <= 140 ms (from ~320 ms baseline). `npm test` green. `npm run balance -- 500
--seed 42` identical before and after the whole plan. `tsc --noEmit`, `npm run lint`, `npm run check:styles`
