...
|---|---|---|---|---|---|
| 3 | [game_canvas](plans/plan_game_canvas_2026-09-15.md) — MTGA-style fixed-resolution scaled canvas for touch devices (zero scroll/clip by construction), supersedes `mobile_responsive`'s remaining fluid-layout scope (its T1-T4/T6 work + manifest/auto-login/OrientationGate is kept, not redone) | planned | — (ui_foundation, deckbuilder_ux, mobile_responsive T1-T4/T6 done) | `GameCanvas.tsx`, `app/layout.tsx`, `OrientationGate.tsx` (merges in), every route's `page.tsx`/screen component (dvh audit), `playwright.config.ts`, `tests/mobile-audit.spec.ts` | 1-2 days |
