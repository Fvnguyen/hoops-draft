...
| 15 | [pvp_match](plans/plan_pvp_match_2026-09-22.md) — Playoffs record: `matches` table + RPCs + RLS, invite any user, accept from the bell, Realtime channel with polling fallback, server-side game simulation route | T1-T6 done, all exit criteria green (migration applied, `playoffs-invite.spec` passes with two accounts); ready for `/roadmap done` | — | `supabase/migrations/202609220001_matches.sql`, `lib/matchChannel.ts`, `hooks/useMatch.ts`, `hooks/useNotices.ts`, `app/api/match/**`, `app/playoffs/new` | 2 days |
