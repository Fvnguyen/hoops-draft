...
| 9 | [challenge_mode](plans/plan_challenge_mode_2026-09-17.md) — 82-game gauntlet against all 30 NBA rosters (built from our own cards) after draft/deckbuild, midseason checkpoint at game 41 with one trade, S-F grade on the full season | planned | — | `engine/challenge.ts` (new), `engine/types.ts`/`balance.ts` additions, `storage/*` (new `ChallengeRun` entity), `components/cardColors.ts` (team-abbr fix), `app/challenge/[rosterId]/` (new), `ChallengeDashboard`/checkpoint/results components (new), `PackOpener.tsx` (`variant="trade"`) | 4-5 days |
