...
| 11 | [mobile_native_feel](plans/plan_mobile_native_feel_2026-09-18.md) — kill browser context menu/double-tap-zoom, guard the Android back button with a save/cancel sheet, tap-anywhere collapsible bars, bigger icon hit areas, drop deck-builder per-move undo toasts | planned | — (mobile_responsive, ui_foundation done) | `app/layout.tsx`, `app/globals.css`, new `useAndroidBackGuard` hook, `TopKPIBand.tsx`/`PlayPanel.tsx`/other collapsible bars, `DeckBuilder.tsx` (toast calls), `Toast.tsx` | 1-2 days |
