...
|---|---|---|---|---|---|
| 3 | [game_canvas](plans/plan_game_canvas_2026-09-15.md) — D0 decided: no scaled canvas. The text-position audit rule + 780x360 project found four real clips at both 830 and 780 (home, draft header, deck-builder band, season dashboard); all fixed in the fluid layout, audit clean at three widths. Only T5 (owner real-device pass) is open; a height-only failure there reopens the canvas | T5 owner | — (ui_foundation, deckbuilder_ux, mobile_responsive T1-T4/T6 done) | `GameCanvas.tsx`, `app/layout.tsx`, `OrientationGate.tsx` (merges in), every route's `page.tsx`/screen component (dvh audit), `playwright.config.ts`, `tests/mobile-audit.spec.ts` | 1-2 days |
