...
| 12 | [sync_outbox](plans/plan_sync_outbox_2026-09-21.md) — sync starts on login without a reload, saves resolve locally and push through a persisted outbox, delete tombstones, deterministic season/challenge ids, RLS approval check; owner applies one migration | planned | — | `src/storage/*`, `AuthProvider`/`StorageProvider`, `(auth)/*`, new migration | 2-3 days |
