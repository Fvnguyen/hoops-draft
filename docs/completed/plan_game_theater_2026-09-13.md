...
File: `docs/completed/plan_game_theater_2026-09-13.md`. Status: done 2026-09-17 (manual
override by owner — closed before its own two exit criteria ran: an in-app season game
with a close finish, and loading a season saved before this plan. T6, removing
`narrativeText` once those pass, was skipped for the same reason and is still open —
`engine/game.ts` still emits it, `narration/render.ts` still falls back to it). Sequence:
was 6 in `docs/ROADMAP.md`. Depends on:
