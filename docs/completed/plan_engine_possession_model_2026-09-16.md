...
- D1 **Standardise inside the engine.** Each rating maps to `50 + 15·(r − mean)/sd` (clamped
  0..99) with per-dimension pool mean/sd (`RATING_NORM`, printed by `npm run build:cards`,
  drift-tested against `cards.json`). Raw ratings, badges and UI untouched. Raw dimensions
  live on different scales (playmaking mean 35 / sd 23.5 vs perimeter defence 53.5 / 16.5),
  so neither k nor a centred edge is comparable without it.
...
- D7 Badges stay out of dimension mechanics (`badge_effects` plan later); nothing here
  reads `traits`.
- D8 Every constant change is one commit quoting before/after (game_engine D7). Baseline
  2026-09-16 (`500 --seed 42`): PPP 1.061, sd 13.2, [90,130] 87.7%, home win 55.0%.
...
Rating formula / OVR-40 floor / rarity bands (`card_balance`, now after this plan); badge
effects (`badge_effects`); narration (`game_theater`); bot drafting (`draft_ai`); fatigue/
fouls/injuries (game_engine D6 still holds).
