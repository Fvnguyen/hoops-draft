# Magic Ball — Code Review and Architecture Proposals (2026-09-12)

> Completed. Phases 0-1 shipped; the phased plan in section 5 was superseded on
> 2026-09-13 by the plan sequence in `docs/ROADMAP.md`. Proposals D, E, G are still the
> reference for the unplanned mobile / accounts / Android plans. File paths in sections
> 2 and 6 refer to `src/lib/*`, which became `src/engine/*` in Phase 1.
...
Order agreed on 2026-09-12: correctness first, then engine isolation, then game
improvements, and only then mobile/PWA, accounts, and Android.
...
Phases 0 and 1 are prerequisites for everything else. Phase 2 deliberately comes before 3 so the mobile UI is built once, against a settled game design.
