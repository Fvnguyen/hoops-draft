...
export function getAllCards(): PlayerCard[] {
    ...
    const globalStats = {
        ...
        dbpmNorm: [] as number[], per_stat: [] as number[], vorpNorm: [] as number[], ts: [] as number[]
    };
    ...
    for (const r of rawScores) {
        ...
        if (r.raw.ts > 0) globalStats.ts.push(r.raw.ts);
    }
    ...
    const b_ts = getBenchmark(globalStats.ts);
    ...
    for (const r of rawScores) {
      ...
      // RESTORE COMPOSITE ADVANCED SCORE MULTIPLIER (PER + VORP)
      const perIndex = getIndex(stat.per, b_per_stat);
      const vorpIndex = getIndex(r.raw.vorpNorm, b_vorpNorm);
      const tsIndex = getIndex(r.raw.ts, b_ts);
      
      // The true composite advanced score: 40% PER, 40% VORP, 20% True Shooting
      const compositeAdvancedScore = (perIndex * 0.40) + (vorpIndex * 0.40) + (tsIndex * 0.20);
