...
export function getAllCards(): PlayerCard[] {
    const RATING_CONFIG = {
        ...
        specialistShift: {
            top1: 0.10, // Max +10% to best skill
            top2: 0.05, // Max +5% to 2nd best
            bottom2: 0.05, // Drain 5% from 5th
            bottom1: 0.10, // Drain 10% from 6th
            minFloor: 0.05 // Weights can never go below 5%
        }
    };
    ...
    for (const r of rawScores) {
      ...
      if (r.pool === "Guards") {
          wFin = 0.15; wMid = 0.15; wPer = 0.25; wPlay = 0.20; wReb = 0.05; wDef = 0.20;
      } else if (r.pool === "Wings") {
          wFin = 0.20; wMid = 0.15; wPer = 0.20; wPlay = 0.10; wReb = 0.15; wDef = 0.20;
      } else {
          wFin = 0.25; wMid = 0.10; wPer = 0.05; wPlay = 0.05; wReb = 0.30; wDef = 0.25;
      }
      
      const combinedDef = (perimeterDefense * 0.6) + (postDefense * 0.4);
      
      const activeWeights = [
          { key: 'fin', val: finishing, weight: wFin },
          { key: 'mid', val: midRange, weight: wMid },
          { key: 'per', val: perimeter, weight: wPer },
          { key: 'ply', val: playmaking, weight: wPlay },
          { key: 'reb', val: rebounding, weight: wReb },
          { key: 'def', val: combinedDef, weight: wDef }
      ];
      
      activeWeights.sort((a, b) => b.val - a.val);
      
      const shift = RATING_CONFIG.specialistShift;
      let totalDrained = 0;
      
      const drain1 = Math.min(shift.bottom1, activeWeights[5].weight - shift.minFloor);
      activeWeights[5].weight -= drain1;
      totalDrained += drain1;
      
      const drain2 = Math.min(shift.bottom2, activeWeights[4].weight - shift.minFloor);
      activeWeights[4].weight -= drain2;
      totalDrained += drain2;
      
      const give1 = Math.min(shift.top1, totalDrained);
      activeWeights[0].weight += give1;
      totalDrained -= give1;
      
      activeWeights[1].weight += totalDrained; 
      
      let baseOvr = 0;
      for (const w of activeWeights) {
          baseOvr += w.val * w.weight;
      }
